...
bool DisableBaggedItems = FALSE;

#define          SWAP_CONTAINERS  false        //Swap containers to cast items

#define       DELAY_CAST    12000
//#define       DELAY_MEMO     6000  //Changed to long variable for toon level adjustments.
...
bool          fINVCLICK=false;         // ItemFound/ItemSearch - Find Item Click from inventory
...
void __stdcall CastEvent(unsigned int ID, void *pData, PBLECHVALUE pValues) {
  ...
  if(DEBUGGING)
    WriteChatf("[%d] MQ2Cast:[OnChat]: Result=[%d] Called=[%d].",(long)GetTickCount(),CastingE,(long)pData);
}
 
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=//
 
void CheckMQ2BagWindow(bool Init)
{
    unsigned int uiLength = strlen("MQ2BagWindow") + 1;
    PMQPLUGIN pLook = pPlugins;
    while (pLook && strnicmp("MQ2BagWindow", pLook->szFilename, uiLength))
    {
        pLook = pLook->pNext;
    }
    bool fCreateBagWnd = 0;
    bool fBagWndOpen = 0;
    if (pLook) fCreateBagWnd = (void (*)(PSPAWNINFO pLPlayer, char* szLine))GetProcAddress(pLook->hModule, "CreateMQ2BagWindow");
    if (fCreateBagWnd) {
        typedef bool (__cdecl *BagCALL) ();
        BagCALL fBagWndOpen=(BagCALL)GetProcAddress(pLook->hModule,"BagWindowOpen");
        if (!fBagWndOpen() && useBagWindow)
        {
            useBagWindow=false;
            DisableBaggedItems=true;
        } else if (fBagWndOpen() && !useBagWindow) {
            DisableBaggedItems=false;
            useBagWindow=true;
        }
        if (!fBagWndOpen() && Init)
        {
            WriteChatf("\ayMQ2Cast\aw:: \arMQ2BagWindow detected, but the window is not loaded.  You need to reload your UI to load the window using /loadskin.  Item casting from inside of bags disabled until the window is loaded.");
            DisableBaggedItems=true;
            useBagWindow=false;
        }
    } else if(!fCreateBagWnd && useBagWindow && !Init) {
        useBagWindow=false;
        DisableBaggedItems=true;
        WriteChatf("\ayMQ2Cast\aw:: \arMQ2BagWindow has been unloaded, disabling item casting.");
    } else if(!fCreateBagWnd && Init) {
        WriteChatf("\ayMQ2Cast\aw:: \arMQ2BagWindow plugin not found.  Item swapping from within bags is disabled.  To enable item swapping in bags please load MQ2BagWindow and MQ2Exchange and then reload MQ2Cast");
        useBagWindow=false;
        DisableBaggedItems=true;
    }
}
...
void Execute(PCHAR zFormat, ...) {
 ...
 void ExecuteDly(PCHAR zFormat, ...) {
  ...
  vsprintf(zOutputDly,zFormat,vaList); if(!zOutputDly[0]) return;
  ulTimer = GetTickCount() + EX_DELAY;
  cPendingEq = true;
  //DoCommand(GetCharInfo()->pSpawn,zOutput);
}

bool Flags() {
    if(!BardClass() && pCastingWnd && (PCSIDLWND)pCastingWnd->dShow) { if(DEBUGGING) WriteChatf("MQ2Cast: pCastingWnd=TRUE"); return true; }
...
long ItemFound(long ID, long B, long E) {
  ...
  for(int iSlot=B; iSlot < E; iSlot++)
    if(PCONTENTS cSlot=GetCharInfo2()->pInventoryArray->InventoryArray[iSlot]) {
      if(ID == GetItemFromContents(cSlot)->ItemNumber) {
        fSLOT=iSlot;
        fITEM=cSlot;
        fPACK=(iSlot>=BAG_SLOT_START)?cSlot:NULL;
        return true;
      } else if (GetItemFromContents(cSlot)->Type == ITEMTYPE_PACK && !DisableBaggedItems) {
        if(!useBagWindow && !bagsOpen)
        {
        ...
        for(int iPack=0; iPack < GetItemFromContents(cSlot)->Slots; iPack++) {
            if(cSlot->pContentsArray) {
              if(PCONTENTS cPack=cSlot->pContentsArray->Contents[iPack]) {
                if(ID == GetItemFromContents(cPack)->ItemNumber) {
                  //fSLOT=(iSlot-BAG_SLOT_START)*10+262+iPack;
                  fSLOT = FindInvSlotForContents(cPack);
                  fITEM=cPack; 
                  fPACK=cSlot;
                  return true;
                }
...
long ItemSearch(PCHAR ID, long B, long E) {
  ...
  for(int iSlot=B; iSlot < E; iSlot++)
    if(PCONTENTS cSlot=GetCharInfo2()->pInventoryArray->InventoryArray[iSlot]) {
      if(!stricmp(ID,GetItemFromContents(cSlot)->Name)) {
        fSLOT=iSlot;
        fITEM=cSlot;
        fPACK=(iSlot>=BAG_SLOT_START)?cSlot:NULL;
        return true;
        ...
        for(int iPack=0; iPack < GetItemFromContents(cSlot)->Slots; iPack++) {
            if(cSlot->pContentsArray) {
              if(PCONTENTS cPack=cSlot->pContentsArray->Contents[iPack]) {
                if(!stricmp(ID,GetItemFromContents(cPack)->Name)) {
                  fSLOT = FindInvSlotForContents(cPack);
                  fITEM=cPack; 
                  fPACK=cSlot;
                  return true;
...
void MemoLoad(long Gem, PSPELL Spell) {
  if(!Spell || Spell->Level[GetCharInfo()->pSpawn->Class-1]>GetCharInfo()->pSpawn->Level) return;
...
bool Moving() {
  long  MyTimer=(long)GetTickCount();
...
BOOL Paused() {
  ...
  if(pLootWnd && (PCSIDLWND)pLootWnd->dShow)         return true;
  if(pBankWnd && (PCSIDLWND)pBankWnd->dShow)         return true;
  if(pMerchantWnd && (PCSIDLWND)pMerchantWnd->dShow) return true;
  if(pTradeWnd && (PCSIDLWND)pTradeWnd->dShow)       return true;
  if(pGiveWnd && (PCSIDLWND)pGiveWnd->dShow)         return true;
...
//  WriteChatf("SlotEquip func - Item %d : CurrSlotID %d : WantSlotID %d : EffectType %d",Item,CurrSlotID,WantSlotID,Effects);
  long MaxSlot=(Effects==1 || Effects==3 || Effects==5)?NUM_INV_SLOTS:BAG_SLOT_START-1;
  //if(CurrSlotID<=MaxSlot) return CurrSlotID;
  if(WantSlotID>NOID && WantSlotID<=MaxSlot) {
    if(WantSlotID>BAG_SLOT_START-1) {
      PCONTENTS pSlot=GetCharInfo2()->pInventoryArray->InventoryArray[WantSlotID];
      if(!pSlot || GetItemFromContents(pSlot)->Type!=ITEMTYPE_PACK) return WantSlotID;
    } else if(Item->EquipSlots&(1<<WantSlotID)) return WantSlotID;
  }
  for(long Desired=BAG_SLOT_START; Desired<MaxSlot; Desired++) {
    //WriteChatf("Slot %d %s",Desired,(GetCharInfo2()->pInventoryArray->Inventory.Pack[Desired-BAG_SLOT_START])?"TRUE":"FALSE");
    PCONTENTS pSlot=GetCharInfo2()->pInventoryArray->InventoryArray[Desired];
    if ((!pSlot && Desired < BAG_SLOT_START+8) || (pSlot && GetItemFromContents(pSlot)->Type!=ITEMTYPE_PACK)) return Desired;
  }
  //return NOID;
  for(long Desired=0; Desired<BAG_SLOT_START; Desired++) {
    if(Item->EquipSlots&(1<<Desired)) return Desired;
  }
  if(SWAP_CONTAINERS) return BAG_SLOT_START+7;
  else return NOID;
}
...
bool SpellFind(PCHAR ID, PCHAR TYPE) {
  ...
  if(ID[0]) {
    ...
    if(ItemSearch(ID,0,NUM_INV_SLOTS)) if(GetItemFromContents(fITEM)->Clicky.SpellID) {
      fFIND=GetSpellByID(GetItemFromContents(fITEM)->Clicky.SpellID);
      fINFO=fITEM;
      fTIME=GetItemFromContents(fITEM)->CastTime;
      fNAME=(PCHAR)GetItemFromContents(fITEM)->Name;
      fTYPE=TYPE_ITEM;
	  BYTE Effect=GetItemFromContents(fITEM)->Clicky.EffectType;
      if (Effect==1 || Effect==5) fINVCLICK=true;
	  else fINVCLICK=false;
      return true;
    }
  }
  fFIND=NULL;
...
bool SpellReady(PCHAR ID) { 
  ...
  if(IsNumber(ID)) {
    ...
    if((DWORD)number<NUM_SPELL_GEMS) return GEMReady(number);
  }
  if(ID[0]=='M' && strlen(ID)==1) return !(Twisting=Evaluate("${If[${Twist.Twisting},1,0]}")?true:false);
  ...
  bool GetMember(MQ2VARPTR VarPtr, PCHAR Member, PCHAR Index, MQ2TYPEVAR &Dest) {
    ...
    if(pMember) switch((CastMembers)pMember->ID) {
      ...
      case Status: 
        ...
        if( CastingC!=NOID ||
            CastF!=FLAG_COMPLETE || 
            (pCastingWnd && (PCSIDLWND)pCastingWnd->dShow)) strcat(Temps,"C");
...
void StopEnding() {
  if(MoveS!=FLAG_COMPLETE) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Immobilize]: Stick UnPause Request.",(long)GetTickCount());
    Stick("unpause");
    MoveS=FLAG_COMPLETE;
  }
  if(MoveA!=FLAG_COMPLETE) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Immobilize]: AdvPath UnPause Request.",(long)GetTickCount());
    Execute("/varcalc PauseFlag 0");
    MoveA=FLAG_COMPLETE;
  }
  if(MoveF!=FLAG_COMPLETE) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Immobilize]: MQ2AdvPath UnPause Request.",(long)GetTickCount());
    FollowPath("unpause");
    MoveF=FLAG_COMPLETE;
  }
...
void StopHandle() {
  if(StopF==FLAG_REQUEST) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Immobilize]: Request.",(long)GetTickCount());
    StopM=(long)GetTickCount()+DELAY_STOP;
    StopF=FLAG_PROGRESS1;
    StopE=DONE_PROGRESS;
  }
  if(Evaluate("${If[${Stick.Status.Equal[ON]},1,0]}")) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Immobilize]: Stick Pause Request.",(long)GetTickCount());
    Stick("pause");
    MoveS=FLAG_PROGRESS1;
  }
  if(Evaluate("${If[${Bool[${FollowFlag}]},1,0]}")) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Immobilize]: AdvPath Pause Request.",(long)GetTickCount());
    Execute("/varcalc PauseFlag 1");
    MoveA=FLAG_PROGRESS1;
  }
  if(Evaluate("${If[${AdvPath.Following} && !${AdvPath.Paused},1,0]}")) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Immobilize]: MQ2AdvPath Pause Request.",(long)GetTickCount());
    FollowPath("pause");
 ...
 if(Evaluate("${If[${AdvPath.Playing} && !${AdvPath.Paused},1,0]}")) {
  ...
  if(Immobile=Moving()) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Immobilize]: Complete.",(long)GetTickCount());
    StopF=FLAG_COMPLETE;
    StopE=DONE_SUCCESS;
  }
  if((long)GetTickCount() > StopM) {
    WriteChatf("[%d] MQ2Cast:[Immobilize]: Aborting!",(long)GetTickCount());
    StopF=FLAG_COMPLETE;
    StopE=DONE_ABORTED;
    return;
  }
  ...
  if(!pSpellBookWnd) MemoE=DONE_ABORTED;
    ...
    if(!Complete) {
      if(MemoF==FLAG_REQUEST) {
        if(DEBUGGING)
          WriteChatf("[%d] MQ2Cast:[Memorize]: Immobilize.",(long)GetTickCount());
        MemoF=FLAG_PROGRESS1; 
        MemoE=DONE_PROGRESS;
        if(GetCharInfo()->pSpawn->Level<4 && DELAY_MEMO<15000) DELAY_MEMO=15000;
        MemoM=(long)GetTickCount()+DELAY_STOP+DELAY_MEMO*SpellTotal;
      ...
      if(MemoF==FLAG_PROGRESS1 && StopE==DONE_SUCCESS) {
        if(DEBUGGING)
          WriteChatf("[%d] MQ2Cast:[Memorize]: Spell(s).",(long)GetTickCount());
        MemoF=FLAG_PROGRESS2;
        DWORD Favorite=(DWORD)&SpellToMemorize;
        pSpellBookWnd->MemorizeSet((int*)Favorite,NUM_SPELL_GEMS);
      }
      if(StopE==DONE_ABORTED || (long)GetTickCount()>MemoM) MemoE=DONE_ABORTED;
    } else {
      if(DEBUGGING)
        WriteChatf("[%d] MQ2Cast:[Memorize]: Complete.",(long)GetTickCount());
      MemoF=FLAG_COMPLETE;
      MemoE=DONE_SUCCESS;
    }
  }
  if(MemoE==DONE_ABORTED || !pSpellBookWnd) {
    WriteChatf("[%d] MQ2Cast:[Memorize]: Aborting!",(long)GetTickCount());
    MemoF=FLAG_COMPLETE;
  }
  if(MemoF==FLAG_COMPLETE && (pSpellBookWnd && (PCSIDLWND)pSpellBookWnd->dShow)) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Memorize]: Closebook.",(long)GetTickCount());
    Execute("/book");
  }
}
...
void ItemHandle(bool SwapIn) {
  ...
  if (CastK==TYPE_ITEM && fINVCLICK)
	  return;
//  if(GetCharInfo2()->Cursor && !SwapIn) WinClick((CXWnd*)pInventoryWnd,"InvSlot30","leftmouseup",1);
  if(!SwapIn && ItemF!=FLAG_COMPLETE && !GetCharInfo()->pSpawn->CastingData.SpellETA) { // && GetTickCount() > ulTimerR) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Swapping]: Out.",(long)GetTickCount());
    for(int X=0;X<NUM_INV_SLOTS;X++) 
      if(ItemA[X]) Exchange("%d %d",ItemA[X],X);
    for(int Y=0;Y<NUM_INV_SLOTS;Y++) 
      if(PCONTENTS Cont=GetCharInfo2()->pInventoryArray->InventoryArray[Y])
        if(ItemA[Y] && GetItemFromContents(Cont)->ItemNumber==ItemA[Y]) ItemA[Y]=0;
    ItemF=FLAG_COMPLETE;
  ...
  if(SwapIn && ItemF==FLAG_COMPLETE && (CastB[0] || CastK==TYPE_ITEM)) {
    ...
    for(int X=0;X<NUM_INV_SLOTS;X++)
      if(PCONTENTS Cont=GetCharInfo2()->pInventoryArray->InventoryArray[X])
        if(GetItemFromContents(Cont)->Type!=ITEMTYPE_PACK)
          ItemA[X]=GetItemFromContents(Cont)->ItemNumber;
    if(CastB[0]) Bandolier(CastB);
    if(CastK==TYPE_ITEM) {
      if(ItemFound(GetItemFromContents(((PCONTENTS)CastI))->ItemNumber,0,NUM_INV_SLOTS)) {
        long wSLOT=SlotID(CastC);
        long eSLOT=SlotEquip(GetItemFromContents(fITEM),fSLOT,wSLOT);
        if(eSLOT==BAG_SLOT_START+7 && !PulseCount) 
            if(PCONTENTS Cont=GetCharInfo2()->pInventoryArray->InventoryArray[eSLOT])
                if(GetItemFromContents(Cont)->Type==ITEMTYPE_PACK)
                    PulseCount=1;
        if(fSLOT!=eSLOT && eSLOT > NOID) Exchange("%d %d",GetItemFromContents(((PCONTENTS)CastI))->ItemNumber,eSLOT);
      }
    }
    for(int Y=0;Y<NUM_INV_SLOTS;Y++)
      if(PCONTENTS Cont=GetCharInfo2()->pInventoryArray->InventoryArray[Y])
        if(GetItemFromContents(Cont)->Type==ITEMTYPE_PACK || GetItemFromContents(Cont)->ItemNumber==ItemA[Y]) ItemA[Y]=0;
        //if(GetItemFromContents(Cont)->Type==ITEMTYPE_PACK) ItemA[Y]=0;
        //if(GetItemFromContents(Cont)->Type==ITEMTYPE_PACK || GetItemFromContents(Cont)->ItemNumber==ItemA[Y]) ItemA[Y]=0;
    for(int Z=0;Z<NUM_INV_SLOTS && ItemF==FLAG_COMPLETE;Z++) if(ItemA[Z]) ItemF=FLAG_PROGRESS1;
    if(ItemF!=FLAG_COMPLETE) {
      if(DEBUGGING)
        WriteChatf("[%d] MQ2Cast:[Swapping]: In.",(long)GetTickCount());
    }
  }
}
...
void DuckHandle() {
  if(DuckF==FLAG_REQUEST) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Duck]: Request.",(long)GetTickCount());
  ...
  if(DuckF==FLAG_PROGRESS1) {
    if(GetCharInfo()->pSpawn->Mount) {
      if(!DuckM) {
        if(DEBUGGING)
          WriteChatf("[%d] MQ2Cast:[Duck]: Dismount.",(long)GetTickCount());
        DuckM=(long)GetTickCount();
        Execute("/dismount");
      }
 ...
 if(DuckF==FLAG_PROGRESS2) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Duck]: StopCast.",(long)GetTickCount());
    Execute("/stopcast");
...
void CastHandle() {
  ...
  if(CastF!=FLAG_COMPLETE) {
    if(StopE==DONE_ABORTED || MemoE==DONE_ABORTED || (long)GetTickCount()>CastM) {
...
PLUGIN_API VOID CastCommand(PSPAWNINFO pChar, PCHAR Cmd) {
  ...
  {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Casting]: Complete. [%d][%s%s%s%s]",(long)GetTickCount(),Resultat,
  ...
  if(Resultat!=CAST_SUCCESS) {
    if(DEBUGGING)
      WriteChatf("[%d] MQ2Cast:[Casting]: Complete. [%d]",(long)GetTickCount(),Resultat);
    return;
  }
  CastF=FLAG_REQUEST;
  CastI=fINFO;
  ...
  strcpy(CastN,fNAME);
  if(DEBUGGING)
    WriteChatf("[%d] MQ2Cast:[Casting]: Name<%s> Type<%d>.",(long)GetTickCount(),CastN,CastK);
  CastHandle();
}
...
PLUGIN_API VOID SpellSetSave(PSPAWNINFO pChar, PCHAR Cmd) {
  ...
  if(!zGem[0]) sprintf(zGem,"123456789ABC");
...
PLUGIN_API VOID InitializePlugin(VOID) {
  ...
  CheckMQ2BagWindow(true);
}

PLUGIN_API VOID OnReloadUI(VOID)
{
    CheckMQ2BagWindow(false);
}
...
PLUGIN_API VOID OnPulse(VOID) {
    ...
    } else if(!fCreateBagWnd && useBagWindow) {
        useBagWindow=false;
        DisableBaggedItems=true;
        WriteChatf("\ayMQ2Cast\aw:: \arMQ2BagWindow has been unloaded, disabling item casting.");
    }*/
  ...
  if(gbInZone && (long)GetTickCount()>CastingP && GetCharInfo() && GetCharInfo()->pSpawn) { 
      CastingP=(long)GetTickCount()+DELAY_PULSE;
    ...
    if(pCastingWnd && (PCSIDLWND)pCastingWnd->dShow) {
      ...
      if(CastingD!=CastingC && CastingD!=NOID) {
        ...
        CastingT=GetCharInfo()->pSpawn->CastingData.SpellETA  - 
                 ...
                 GetTickCount()+450+(pConnection->Last)*4;
    ...
    if(Casting) {
      ...
      CastingT=GetTickCount()+450+(pConnection->Last)*2;
      Casting=false;
    }
    if(CastingE==DONE_PROGRESS) {
      if((long)GetTickCount()>CastingT) CastingE=CAST_SUCCESS;
      else if(!Twisting) return;
    }
    if(Paused()) {
      if((long)GetCharInfo()->pSpawn->CastingData.SpellID>0) Execute("/stopsong");
      return;
    }
    ...
    if((CastF==FLAG_COMPLETE && CastingC!=NOID && CastingD==NOID) ||
      ...
      if(!Twisting) {
        if(DEBUGGING)
          WriteChatf("[%d] MQ2Cast:: Casting Complete ID[%d] Result=[%d]",(long)GetTickCount(),CastingL,CastingX);
    ...
    if(CastF==FLAG_COMPLETE) {
      ...
      StopEnding();
        if(PulseCount) {
            ...
            if(GetCharInfo2()->pInventoryArray->Inventory.Cursor && PulseCount) ClickBack();
            if(PulseCount && PulseCount<7) PulseCount++;
        }
    }
  }
}
